(() => {
  ...
  function renderField(ph) {
    ...
    // 1. FECHA DE SOLICITUD: OCULTARLA POR COMPLETO DEL FORMULARIO
    // Usamos una regex flexible para detectar el campo de solicitud
    if (isFecha(ph) && /solicitud/i.test(ph)) {
        return ''; // No renderiza nada, el campo desaparece visualmente
    }
    ...
    // 6. VALIDACIÓN FLEXIBLE DE RFC (Homoclave opcional)
    if (isRfc(ph)) {
    ...
    // 7. OTRAS FECHAS (Inicio, Fin, Baja, etc.)
    if (isFecha(ph)) {
  ...
  // ... (El resto del código de CSV helpers se mantiene igual) ...
  ...
  // --- DISPLAY DE FECHA (ESTÉTICO - Grande y alineado) ---
  function renderDateLabel() {
    // Busca el título H1 o inserta antes del contenedor si no hay h1
    const titleEl = document.querySelector('h1') || document.querySelector('header');
    
    // Elimina fecha previa si existe (para evitar duplicados al recargar)
    ...
    badge.innerHTML = `<strong>📅 ${dateStr}</strong>`; // Texto simplificado y directo
    
    // Estilos CSS inline: Grande, elegante, alineado
    badge.style.cssText = `
        font-size: 1.25rem; 
        color: #1f2937; 
        background: #f3f4f6; 
    ...
    if (titleEl) {
        // Aseguramos que el título y la fecha estén en la misma línea visualmente
        // Hacemos el título flex para alinear
  ...
  async function generar() {
    ...
    for (const el of inputs) {
        // Validar SOLO campos visibles (excluyendo hidden y los que no están en el DOM)
    ...
    // --- INYECCIÓN AUTOMÁTICA DE FECHA ---
    // Como el campo no existe en el DOM, lo agregamos aquí manualmente.
    const now = new Date();
    const dd = String(now.getDate()).padStart(2, '0');
    const mm = String(now.getMonth() + 1).padStart(2, '0');
    const yyyy = now.getFullYear();
    
    // IMPORTANTE: Se envía con la clave exacta que espera el HTML: {{FECHA_SOLICITUD}}
  ...
  function init() {
    ...
    loadTemplates();
    renderDateLabel(); // Mostrar la fecha estética al iniciar
